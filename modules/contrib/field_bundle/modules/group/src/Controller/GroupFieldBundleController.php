...
namespace Drupal\group_field_bundle\Controller;
...
/**
 * Returns responses for 'group_field_bundle' GroupContent routes.
 */
class GroupFieldBundleController extends GroupContentController {
  ...
  /**
   * Constructs a new GroupFieldBundleController.
   *
  ...
  public function addPage(GroupInterface $group, $create_mode = FALSE) {
    ...
    $storage_handler = $this->entityTypeManager->getStorage('field_bundle_config');
    foreach ($this->addPageBundles($group, $create_mode) as $plugin_id => $bundle_name) {
      if (!empty($build['#bundles'][$bundle_name])) {
        ...
        $t_args = ['%field_bundle_config' => $bundle_label];
        $description = $create_mode
          ? $this->t('Add new %field_bundle_config field bundle to the group.', $t_args)
          : $this->t('Add existing %field_bundle_config field bundle to the group.', $t_args);
  ...
  protected function addPageBundles(GroupInterface $group, $create_mode) {
    ...
    // Retrieve all group_field_bundle plugins for the group's type.
    $plugin_ids = $this->pluginManager->getInstalledIds($group->getGroupType());
    foreach ($plugin_ids as $key => $plugin_id) {
      if (strpos($plugin_id, 'group_field_bundle:') !== 0) {
